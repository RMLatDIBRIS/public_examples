...
match(_event, acqRel_et(Id)) :- match(_event, acquire_et(Id)).
match(_event, acqRel_et(Id)) :- match(_event, release_et(Id)).
...
trace_expression('Main', Main) :- (Main=((relevant_et>>Resources);1)),
	(Resources=optional(var(id, (acquire_et(var(id))*((Resources|(star(use_et(var(id)))*release_et(var(id))))/\((acqRel_et(var(id))>>(release_et(var(id))*1));1)))))).
