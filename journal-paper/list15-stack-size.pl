...
trace_expression('Main', Main) :- (Stack=gen([s], ((size_et(var(s))*app(Stack, [var(s)]))\/optional(var(val, (push_et(var(val))*(app(Stack, [(var(s) + 1)])*(pop_et(var(val))*app(Stack, [var(s)]))))))))),
	(Main=((relevant_et>>app(Stack, [0]));1)).
