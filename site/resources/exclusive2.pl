...
match(_event, notAcq_et) :- not(match(_event, acquire_et(_, _))).
...
trace_expression('Main', Main) :- (Main=((relevant_et>>(UseAfterAcquire/\NoMultipleAcquire));1)),
	(UseAfterAcquire=optional(var(eid, var(rid, (acquire_et(var(eid), var(rid))*((star(use_et(var(eid), var(rid)))*release_et(var(eid), var(rid)))|UseAfterAcquire)))))),
	(NoMultipleAcquire=(star(notAcq_et)*optional(var(eid, var(rid, (acquire_et(var(eid), var(rid))*((star(notAcqRel_et(var(eid), var(rid)))*(release_et(var(eid), var(rid))*1))/\NoMultipleAcquire))))))).
