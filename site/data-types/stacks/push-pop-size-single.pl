...
trace_expression('Main', Main) :- (Main=((relevant>>clos(app(Stack, [0])));1)),
	(Stack=gen([s], ((size(var(s))*app(Stack, [var(s)]))\/optional(var(val, (push(var(val))*(app(Stack, [(var(s) + 1)])*(pop(var(val))*app(Stack, [var(s)]))))))))).
