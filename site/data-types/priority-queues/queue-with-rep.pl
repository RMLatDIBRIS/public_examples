...
match(_event, deq_geq(Val)) :- match(_event, deq(Val2)),
	{(Val2 >= Val)}.
match(_event, enq_deq_geq(Val)) :- match(_event, enq(Val)).
match(_event, enq_deq_geq(Val)) :- match(_event, deq_geq(Val)).
...
trace_expression('Main', Main) :- (Main=((relevant>>clos((Queue/\Sorted)));1)),
	...
	(Sorted=(var(val, (enq(var(val))*(((enq_deq_geq(var(val))>>(app(Cons, [var(val)])*1));1)/\Sorted)))\/(deq*Sorted))),
	(Cons=gen([val], (deq(var(val))\/(enq(var(val))*(deq(var(val))|app(Cons, [var(val)])))))).
