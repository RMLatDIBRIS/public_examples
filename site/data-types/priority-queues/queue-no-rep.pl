...
trace_expression('Main', Main) :- (Main=((relevant_et>>clos((Queue/\Sorted)));1)),
	(Queue=var(val, (enq_et(var(val))*((star(enq_et(var(val)))*deq_et(var(val)))|Queue)))),
	(Sorted=(var(val, (enq_et(var(val))*(((deq_geq_et(var(val))>>(deq_et(var(val))*1));1)/\Sorted)))\/(deq_et*Sorted))).
