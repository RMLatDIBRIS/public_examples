...
match(_event, peek) :- match(_event, peek(_)).
match(_event, peek_deq) :- match(_event, peek).
match(_event, peek_deq) :- match(_event, deq).
...
trace_expression('Main', Main) :- (Main=((relevant>>clos(Queue));1)),
	(Queue=var(val, (enq(var(val))*(((star(peek)*deq)|Queue)/\((peek_deq>>(star(peek(var(val)))*(deq(var(val))*1)));1))))).
