...
match(_event, peek_et) :- match(_event, peek_et(_)).
match(_event, peek_deq_et) :- match(_event, peek_et).
match(_event, peek_deq_et) :- match(_event, deq_et).
...
trace_expression('Main', Main) :- (Main=((relevant_et>>clos(Queue));1)),
	(Queue=var(val, (enq_et(var(val))*(((star(peek_et)*deq_et)|Queue)/\((peek_deq_et>>(star(peek_et(var(val)))*(deq_et(var(val))*1)));1))))).
